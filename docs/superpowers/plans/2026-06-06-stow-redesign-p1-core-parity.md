...
- [x] **Step 1: Write `Card.tsx`** (`cardStyle` surface, contract §7 `{ children, onClick?, style?, as? }`)
...
- [x] **Step 2: Write `Button.tsx`** (contract §7: `variant: "primary"|"neutral"|"danger"|"ghost"`; primary = accent bg, #fff text)
...
- [x] **Step 3: Write `Field.tsx`** (contract §7: labeled input, `multiline?` → textarea)
...
- [x] **Step 4: Write `Chip.tsx`** (contract §7: `{ label, selected?, onClick?, color?, onRemove? }`)
...
- [x] **Step 5: Write `ProgressBar.tsx`** (contract §7: `{ value, total }`, accent fill, success at 100%)
...
- [x] **Step 6: Write `RoleBadge.tsx`** (contract §7: OWNER accent / ADMIN success / MEMBER warm — port `prototype/components.jsx` `RoleBadge`)
...
- [x] **Step 7: Write `ItemRow.tsx`** (contract §7: `{ item: Item; onClick?; right?: ReactNode }`, thumb/glyph + name + `room›area` subtitle)
...
- [x] **Step 8: Write `AreaCard.tsx`** (contract §7: `{ name, count?, onClick?, onMenu?() }`)
...
- [x] **Step 9: Write `ResultRow.tsx`** (contract §7: `{ item: Item; query?: string; onClick? }`)
...
Implementation note: this task also re-exported `Box`, `Folder`, and `Save` from `theme/icons.tsx` so the component and later screen imports shown in this plan resolve directly from the canonical icon module.
